...
test('Test +0.01 button on CSGOEmpire roulette page', async ({ page }) => {
  ...
  const plusButton = await page.waitForSelector('button[data-v-a3a19079]:has-text("+ 10")', { state: 'visible', timeout: 10000 });
  ...
  // Verify that the input box value has been updated to "10"
  expect(updatedValue).toBe('10');
});
