...
test('Test +0.01 button on CSGOEmpire roulette page', async ({ page }) => {
  ...
  // Find and click the "+0.01" button
  const plusButton = await page.waitForSelector('button[data-v-a3a19079]:has-text("+ 0.01")', { state: 'visible', timeout: 10000 });
  ...
  // Verify that the input box value has been updated to "0.01"
  expect(updatedValue).toBe('0.01');
});
