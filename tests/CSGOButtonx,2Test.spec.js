...
test('Test +1 and x2 buttons on CSGOEmpire roulette page', async ({ page }) => {
  ...
  // Find the "x2" button
  const doubleButton = await page.waitForSelector('button[data-v-a3a19079].bet-input__control:has-text("x 2")', { state: 'visible', timeout: 10000 });

  // Verify that the "x2" button is clickable
  expect(doubleButton).not.toBeNull();

  // Click the "x2" button
  await doubleButton.click();

  // Verify that the value in the input box is set to "2"
